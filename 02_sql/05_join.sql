...
select emp.emp_id,emp.emp_name,emp.hire_date,dept.dept_name from emp inner join dept on emp.dept_id = dept.dept_id;
...
select emp.emp_id,emp.emp_name,emp.salary,emp.comm_pct,dept.dept_name,dept.loc from emp inner join dept on emp.dept_id = dept.dept_id order by emp.emp_id desc;

-- 직원의 ID(emp.emp_id)가 100인 직원의 직원_ID(emp.emp_id), 이름(emp.emp_name), 입사년도(emp.hire_date), 소속부서이름(dept.dept_name)을 조회.
select emp.emp_id,emp.emp_name,emp.hire_date,dept.dept_name from emp inner join dept on emp.dept_id = dept.dept_id where emp.emp_id=100;
...
select emp.emp_id,emp.emp_name,emp.salary,job.job_title,dept.dept_name 
 ...
 where emp.emp_id=200;

-- 부서_ID(dept.dept_id)가 30인 부서의 이름(dept.dept_name), 위치(dept.loc), 그 부서에 소속된 직원의 이름(emp.emp_name)을 조회.
select dept.dept_name,dept.loc,emp.emp_name
  from emp inner join dept on emp.dept_id = dept.dept_id 
 where dept.dept_id=30;

-- 직원의 ID(emp.emp_id), 이름(emp.emp_name), 급여(emp.salary), 급여등급(salary_grade.grade) 를 조회. 
select emp.emp_id,emp.emp_name,emp.salary,salary_grade.grade
  from emp inner join salary_grade on emp.salary between salary_grade.low_sal and salary_grade.high_sal;

-- 부서별 급여(salary)의 평균을 조회. 부서이름(dept.dept_name)과 급여평균을 출력. 급여 평균이 높은 순서로 정렬. 
select format(avg(emp.salary),2),dept.dept_name
  from emp inner join dept on emp.dept_id = dept.dept_id 
...
select emp.emp_id,emp.emp_name,emp.salary,salary_grade.grade,job.job_title,dept.dept_name 
  from emp inner join dept on emp.dept_id = dept.dept_id 
		   inner join job on emp.job_id = job.job_id
           inner join salary_grade on emp.salary between salary_grade.low_sal and salary_grade.high_sal;
...
select emp.emp_id,emp.emp_name,emp.dept_id,dept.dept_name,dept.loc
 from emp left outer join dept on emp.dept_id = dept.dept_id and dept.dept_id = 80;
...
select emp.emp_id,emp.emp_name
 from emp left outer join job on emp.job_id = job.job_id where emp.emp_id in (100,110,120,130,140);


-- 부서 ID(dept.dept_id), 부서이름(dept.dept_name)과 그 부서에 속한 직원들의 수를 조회. 직원이 없는 부서는 0이 나오도록 조회하고 직원수가 많은 부서 순서로 조회.
select dept.dept_id , dept.dept_name , count(*)
  from dept left outer join emp on dept.dept_id = emp.dept_id
...
select e.emp_id,e.emp_name , m.emp_name , DATE_FORMAT(e.hire_date,'%Y/%m/%d')
  from emp e left outer join emp m on e.mgr_id = m.emp_id
			 left outer join dept on e.dept_id = dept.dept_id
 where e.dept_id = 90;
...
select e.emp_id,e.emp_name,e.salary,e.hire_date,e.salary,job.job_title,m.emp_name,m.hire_date,dept.dept_name,dept.loc
  from emp e left outer join emp m on e.mgr_id = m.emp_id
			 left outer join dept on e.dept_id = dept.dept_id
             left outer join job on e.job_id = job.job_id
