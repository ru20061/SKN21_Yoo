...
select upper(emp_name),lower(emp_name),length(emp_name) from emp;
...
select emp_name , length(emp_name) from emp where length(emp_name)>=10;
...
select emp_id,emp_name,salary,ceil(salary*1.15) SAL_RAISE , ceil(salary*0.15) from emp;
...
select emp_id,emp_name,comm_pct,round(comm_pct*1.08,2) comm_raise로  from emp;
...
select subdate(hire_date,INTERVAL 10 day),hire_date,adddate(hire_date,INTERVAL 10 day) from emp where dept_name in ('IT');
...
select emp_name,timestampdiff(month,hire_date,now()) emp_date from emp order by emp_date;
...
select emp_id,emp_name,job,ifnull(dept_name,'배치 전') from emp;
...
select emp_id , emp_name , case  when job = 'AD_PRES'  then '대표'
...
select dept_name , case when dept_name = 'IT' then salary * 1.1
						when dept_name = 'Shipping' then salary * 1.2
                        when dept_name = 'Finance' then salary * 1.3
                        else 0
					end
 from emp;
