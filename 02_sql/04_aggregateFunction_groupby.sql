...
select count(*) from emp where comm_pct is not null;
...
select year(hire_date),avg(salary) from emp group by year(hire_date);

-- 부서명(dept_name) 이 'Sales'이거나 'Purchasing' 인 직원들의 업무별 (job) 직원수를 조회. 직원수가 많은 순서대로 정렬.
select job,count(*) from emp where dept_name in ('Sales','Purchasing') group by job;

-- 부서(dept_name), 업무(job) 별 최대급여(salary)와 최소급여를 조회.
select min(salary),max(salary) from emp group by dept_name,job;

-- 급여(salary)가 10,000 이하인 직원수와 10,000초과인 직원수 조회.
select case when salary>10000 then '10,000초과'
			when salary<=10000 then '10,000이하' 
            end ,count(*)
	from emp 
    group by case when salary>10000 then '10,000초과'
			when salary<=10000 then '10,000이하' 
            end; 

-- 부서명(dept_name), 업무(job)별 직원수, 최고급여(salary)를 조회. 부서이름으로 오름차순 정렬.
select count(*) , max(salary) from emp group by dept_name, job order by dept_name asc;

-- 부서별(dept_name) 직원수 조회하는데 부서명(dept_name)이 null인 것은 제외하고 조회.
select dept_name,count(*) from emp where dept_name is not null group by dept_name;
...
select year(hire_date),avg(salary) , count(*) from emp group by year(hire_date) having count(*)>=20;
...
select dept_name  from emp group by dept_name having avg(salary)>=5000 and count(*)>=10;  

--  커미션이 있는 직원들의 입사년도별 평균 급여를 조회. 단 평균 급여가 $9,000 이상인 년도분만 조회.

select year(hire_date),avg(salary) from emp where comm_pct is not null group by year(hire_date) having avg(salary)>=9000; 
