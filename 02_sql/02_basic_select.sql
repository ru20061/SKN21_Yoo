...
select emp_id as 직원ID,emp_name as 직원이름,salary as 급여,salary*12 as 연봉 from emp;

-- EMP 테이블의 직원의 이름(emp_name), 급여(salary)를 조회하는데 급여 앞에 '$'를 붙여서 조회한다.
select emp_name as 직원이름,concat(salary,'$') as 급여 from emp;

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션_PCT(comm_pct), 급여에 커미션_PCT를 곱한 값을 조회.
select emp_id as 직원ID,emp_name as 직원이름,salary as 급여,comm_pct as 커미션_PCT,salary*comm_pct from emp;
...
select emp_id,emp_name from emp where emp_name in ('%');

-- EMP 테이블에서 부서명(dept_name)이 null인 직원의 ID(emp_id), 이름(emp_name), 부서명(dept_name)을 조회.
select emp_name , dept_name from emp where dept_name is null;
...
select * from emp where job ='SA_REP' and salary = 9000;

-- EMP 테이블에서 업무(job)가 'FI_ACCOUNT' 거나 급여(salary)가 $8,000 이상인 직원들을 조회.
select * from emp where job ='FI_ACCOUNT' or  salary >= 8000;

-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중에서 부서(dept_name)가 'Shipping' 이고 2005년이후 입사한  직원들을 조회.
select * from emp where job like '%MAN%' and  dept_name = 'Shipping' and year(hire_date) >= 2005 ;
