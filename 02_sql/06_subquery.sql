...
select emp_id , emp_name , job_id , salary
  from emp
where job_id in (select job_id from emp where emp_id = 200);
...
select emp_id , emp_name , job_id , dept_id
  from emp
where (job_id,dept_id) in (select job_id,dept_id from emp where emp_id = 115);
...
select emp_id,emp_name,job_id,mgr_id
  from emp
 where (job_id,mgr_id) in (select job_id,mgr_id from emp where emp_id=150); 
...
select dept.dept_name ,concat(format(emp.salary,2),'$')
...
select dept.dept_name ,dept.loc
  from emp left outer join dept on emp.dept_id = dept.dept_id
...
select *
  from emp 
where salary > (select avg(salary) from emp where dept_id = (select dept_id from dept where dept.dept_name = 'Sales'));
